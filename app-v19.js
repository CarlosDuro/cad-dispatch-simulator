...
function switchView(v,titleOverride){
 const target=$(`#view-${v}`);
 if(!target)return;
 $$(".nav[data-view]").forEach(n=>n.classList.toggle("active",n.dataset.view===v));
 $$(".workspace-tab[data-view]").forEach(t=>t.classList.toggle("active",t.dataset.view===v));
 $$(".view").forEach(x=>x.classList.remove("active"));
 target.classList.add("active");
 const titles={dispatch:"Dispatch Console",dashboard:"Calltaker",map:"Map",people:"Calltaker Console",reports:"Reportes"};
 $("#currentTitle").textContent=titleOverride||titles[v]||v;
 if(location.hash!==`#${v}`)history.replaceState(null,"",`#${v}`);
 setTimeout(()=>{
  dispatchMap?.invalidateSize(true);
  fullMap?.invalidateSize(true);
  renderMaps();
 },250);
}
$$(".nav[data-view]").forEach(n=>n.onclick=()=>switchView(n.dataset.view));
$$(".workspace-tab[data-view]").forEach(t=>t.onclick=()=>switchView(t.dataset.view,t.dataset.title));
...
document.querySelector(".workspace-focus")?.addEventListener("click",async()=>{
 try{
...
setInterval(()=>{
 ...
 data.units.forEach(u=>{
  if(u.operationalState==="en_ruta"){
   const latDiff=incident.lat-u.lat,lngDiff=incident.lng-u.lng;
   const distance=Math.hypot(latDiff,lngDiff);
   if(distance>0.00018){
    const step=Math.min(0.00032,distance*0.18);
    u.lat+=(latDiff/distance)*step;
    u.lng+=(lngDiff/distance)*step;
    u.heading=(Math.atan2(lngDiff,latDiff)*180/Math.PI+360)%360;
    u.speed=28+Math.round(Math.random()*18);
   }else{
    u.operationalState="en_escena";
    u.speed=0;
    addTimeline(incident,`Unidad ${u.id} llegó a la escena`);
   }
  }else if(u.operationalState==="disponible"){
   const angle=(u.heading||0)*Math.PI/180;
   const step=0.00010+Math.random()*0.00008;
   u.lat+=Math.cos(angle)*step+(Math.random()-.5)*0.00004;
   u.lng+=Math.sin(angle)*step+(Math.random()-.5)*0.00004;
   u.heading=((u.heading||0)+(Math.random()-.5)*24+360)%360;
   u.speed=18+Math.round(Math.random()*22);
  }
 });
 save();
 renderMaps();
...
window.addEventListener("DOMContentLoaded",()=>{
 document.body.classList.add("light-suite");
 ...
 const initialView=location.hash.replace("#","")||"dispatch";
 switchView(["dispatch","dashboard","map","people","reports"].includes(initialView)?initialView:"dispatch");
...
window.addEventListener("hashchange",()=>{
 ...
 if(["dispatch","dashboard","map","people","reports"].includes(v))switchView(v);
});
